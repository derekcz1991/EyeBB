...
@SuppressLint("NewApi")
public class BindingChildMacaronActivityBackup extends Activity {
	...
	private final static String TAG = BindingChildMacaronActivityBackup.class
			.getSimpleName();
		...
		protected void onPostExecute(String result) {
			...
			if (result.length() > 0) {
				...
				if (result.equals(HttpConstants.SERVER_RETURN_USED)) {
					Toast.makeText(BindingChildMacaronActivityBackup.this,
			...
			if (result.length() > 0) {
				...
				if (result.equals("T")) {
					...
					DBChildren.updateMacAddressByChildId(
							BindingChildMacaronActivityBackup.this, childId,
							mDeviceAddress);
				} else {
					bindStep = BIND_STEP_UPLOAD_FAIL;
					tvMessage.setText(R.string.text_update_server_data_fail);
